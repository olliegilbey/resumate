...
# Verify logs exist
if [[ ! -f "$RUST_LOG" ]]; then
  echo "❌ Rust test log not found: $RUST_LOG"
  echo "   Run 'just test' to generate logs"
  exit 1
fi

if [[ ! -f "$TS_LOG" ]]; then
  echo "❌ TypeScript test log not found: $TS_LOG"
  echo "   Run 'just test' to generate logs"
  exit 1
fi

echo "📊 Parsing test logs to generate METRICS.md..."
...
echo "✅ METRICS.md generated at $TIMESTAMP"
echo ""
echo "📊 Test Summary:"
