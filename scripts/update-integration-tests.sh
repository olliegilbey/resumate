#!/usr/bin/env bash
# Script to update integration tests to use new schema field names

set -e

echo "🔧 Updating integration tests to use new schema..."

# Backup files
cp doc-gen/crates/core/tests/integration_test.rs doc-gen/crates/core/tests/integration_test.rs.bak
cp doc-gen/crates/core/tests/roundtrip.rs doc-gen/crates/core/tests/roundtrip.rs.bak

# Fix integration_test.rs
sed -i '' 's/resume\.role_profiles/resume.role_profiles.as_ref().unwrap()/g' doc-gen/crates/core/tests/integration_test.rs

# Fix roundtrip.rs - this one needs more extensive changes
cat > doc-gen/crates/core/tests/roundtrip.rs << 'EOF'
...
echo "✅ Integration tests updated"
