...
describe("parseAIOutput", () => {
  it("parses valid JSON with all fields", () => {
    ...
    expect(result.data?.reasoning).toBe("Selected based on relevance to job requirements");
  ...
  it("returns E005 for invalid bullet IDs", () => {
    const raw = JSON.stringify({
      bullets: [b("company-a-pos-1-bullet-1"), b("nonexistent-bullet"), b("also-fake")],
      reasoning: "Test",
    });
