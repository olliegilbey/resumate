/**
 * System prompt for AI bullet scoring.
 *
 * Extracted from the original `prompts/prompt.ts` module. The long template
 * string lives here, along with helpers that identify the prompt version
 * without exposing its full text in analytics payloads.
 *
 * @module lib/ai/prompts/system-prompt
 */

/**
 * System prompt sent with every AI scoring request.
 *
 * Defines the AI role, scoring rubric, output schema, and constraints. Updating
 * this prompt changes analytics payloads (via {@link getSystemPromptHash}) so
 * you can bucket event data by prompt version.
 */
...
/**
 * Load the system prompt string.
 *
 * Exported as a function for backwards compatibility with callers that expect
 * a lazy loader. Equivalent to reading {@link SYSTEM_PROMPT} directly.
 */
...
/**
 * Short hash of the system prompt for analytics.
 *
 * Uses first 8 chars of SHA-256 to identify prompt version without storing the
 * full text. Pair with event data to bucket analytics by prompt revision.
 */
