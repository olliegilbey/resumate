...
export function generateVCard(data: VCardData): string {
  ...
  const displayName = data.nickname ? `${data.nickname} ${data.lastName}`.trim() : data.fullName;
  ...
  if (data.address) {
    ...
    lines.push(
      `ADR;TYPE=WORK:;;${escapedStreet};${escapedCity};${escapedRegion};${escapedPostalCode};${escapedCountry}`,
    );
  }
