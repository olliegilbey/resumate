...
describe("/api/resume/select - Scoring Algorithm (Unit Tests)", () => {
  ...
  it("combines scoring components correctly", () => {
    ...
    const baseScore =
      tagRelevance * scoringWeights.tagRelevance + priorityScore * scoringWeights.priority;
...
describe("/api/resume/select - Test Data Validation", () => {
  ...
  it("role profiles have required fields", () => {
    for (const profile of mockResumeData.roleProfiles) {
      ...
      const sum = profile.scoringWeights.tagRelevance + profile.scoringWeights.priority;
...
describe("/api/resume/select - Scoring Behavior", () => {
  it("higher priority bullets get higher scores (all else equal)", () => {
    ...
    const score1 =
      tagScore * scoringWeights.tagRelevance + (bullet1.priority / 10) * scoringWeights.priority;
    const score2 =
      tagScore * scoringWeights.tagRelevance + (bullet2.priority / 10) * scoringWeights.priority;
  ...
  it("better tag match gets higher score (all else equal)", () => {
    ...
    const score1 =
      tagScore1 * scoringWeights.tagRelevance + priorityScore * scoringWeights.priority;
    const score2 =
      tagScore2 * scoringWeights.tagRelevance + priorityScore * scoringWeights.priority;
