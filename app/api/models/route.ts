...
export async function GET(): Promise<NextResponse<{ models: ModelAvailability[] }>> {
  ...
  const models: ModelAvailability[] = (
    ...
    if (config.provider === 'cerebras') {
      if (!apiKey) {
        return { id, label: config.label, cost: config.cost, available: false, reason: 'API key not configured' }
      }
    ...
    return { id, label: config.label, cost: config.cost, available: false, reason: 'Unknown provider' }
  })
