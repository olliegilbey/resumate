...
fn test_roundtrip_resume_data_template() {
    ...
    let resume_data: ResumeData = serde_json::from_str(&json_content)
        .unwrap_or_else(|e| panic!("Failed to deserialize template JSON: {}\nJSON: {}", e, json_content));

    // Re-serialize back to JSON
    let reserialized = serde_json::to_string_pretty(&resume_data)
        .expect("Failed to re-serialize ResumeData");

    // Parse both as serde_json::Value for semantic comparison
    let original_value: serde_json::Value = serde_json::from_str(&json_content)
        .expect("Failed to parse original JSON");
    let reserialized_value: serde_json::Value = serde_json::from_str(&reserialized)
        .expect("Failed to parse reserialized JSON");
    ...
    assert_eq!(
        ...
        "Roundtrip failed: JSON structure changed during Rust serialization.\n\
         Original fields: {:?}\n\
         Reserialized fields: {:?}",
        original_value.as_object().map(|o| o.keys().collect::<Vec<_>>()),
        reserialized_value.as_object().map(|o| o.keys().collect::<Vec<_>>())
    );
}
...
fn test_resume_data_required_fields() {
    ...
    assert!(!resume_data.personal.name.is_empty(), "personal.name is required");
    ...
    assert!(!resume_data.companies.is_empty(), "companies array should have at least one entry in template");
    ...
    assert!(!first_company.positions.is_empty(), "company.positions should have at least one entry");
    ...
    assert!(!first_position.description.is_empty(), "position.description is required");
}
...
fn test_camel_case_field_names() {
    ...
    let resume_data: ResumeData = serde_json::from_str(&json_content)
        .expect("Failed to deserialize template");
    let reserialized = serde_json::to_string(&resume_data)
        .expect("Failed to re-serialize");

    // Verify camelCase field names are preserved
    assert!(reserialized.contains("\"dateRange\""), "Should use camelCase 'dateRange'");
    assert!(reserialized.contains("\"fullName\""), "Should use camelCase 'fullName'");
    assert!(reserialized.contains("\"descriptionTags\""), "Should use camelCase 'descriptionTags'");
    assert!(reserialized.contains("\"descriptionPriority\""), "Should use camelCase 'descriptionPriority'");

    // Verify snake_case is NOT present
    assert!(!reserialized.contains("\"date_range\""), "Should NOT use snake_case 'date_range'");
    assert!(!reserialized.contains("\"full_name\""), "Should NOT use snake_case 'full_name'");
    assert!(!reserialized.contains("\"description_tags\""), "Should NOT use snake_case 'description_tags'");
}
